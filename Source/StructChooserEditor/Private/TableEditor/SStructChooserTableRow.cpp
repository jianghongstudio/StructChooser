...
#include "SStructChooserRowHandle.h"
...
namespace UE::StructChooserEditor
{
	...
	void SStructChooserTableRow::Construct(const FArguments& Args, const TSharedRef<STableViewBase>& OwnerTableView)
	{
		...
		SMultiColumnTableRow<TSharedPtr<FStructChooserTableRow>>::Construct(
	...
	TSharedRef<SWidget> SStructChooserTableRow::GenerateWidgetForColumn(const FName& ColumnName)
	{
		...
		if (Chooser->ResultsStructs.IsValidIndex(RowIndex->RowIndex))
		{
			if (ColumnName == AddColumn || ColumnName == Handles)
			{
				...
				return SNew(SOverlay)
						+ SOverlay::Slot()
						[
							SNew(SStructChooserRowHandle, bShowHandleImage).ViewModel(ChooserViewModel).RowIndex(RowIndex->RowIndex)
						]
						+ SOverlay::Slot()
						[
			...
			else if (ColumnName == Result) 
			{
				...
				TSharedPtr<SWidget> ResultWidget = CreateStructChooserResultCellWidget(
					false,
					Chooser,
					&Chooser->ResultsStructs[RowIndex->RowIndex],
			...
			if (ColumnName == Handles)
			{
				...
				[
					SNew(SBox).Padding(0.0f) .HAlign(HAlign_Center) .VAlign(VAlign_Center) .WidthOverride(16.0f)
                    					[
                    						SNew(SImage)
                    						.Image(FSlateStyleRegistry::FindSlateStyle(TEXT("ChooserEditorStyle"))
							? FSlateStyleRegistry::FindSlateStyle(TEXT("ChooserEditorStyle"))->GetBrush(TEXT("ChooserEditor.FallbackIcon"))
							: nullptr)
		...
		else if (RowIndex->RowIndex == SpecialIndex_AddRow)
		{
			...
			if (bIsLeftmostColumn)
			{
				return SNew(SHorizontalBox)
								...
								[
									SNew(SStructChooserCreateRowButton).ViewModel(ChooserViewModel)
								];
			}
		}
	...
	void SStructChooserTableRow::OnDragEnter(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
	{
	...
	void SStructChooserTableRow::OnDragLeave(const FDragDropEvent& DragDropEvent)
	{
		bDragActive = false;
	}

	FReply SStructChooserTableRow::OnDragOver(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
	{
		DragActiveCounter = 2;
		bDragActive = true;
	...
	FReply SStructChooserTableRow::OnDrop(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
	{
		bDragActive = false;
	...
	void SStructChooserTableRow::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
	{
		DragActiveCounter --;
		...
		SMultiColumnTableRow<TSharedPtr<FStructChooserTableRow, ESPMode::ThreadSafe>>::Tick(AllottedGeometry, InCurrentTime, InDeltaTime);
	}
}
