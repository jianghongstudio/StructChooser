...
namespace UE::StructChooserEditor
{
	...
	TSharedRef<ITableRow> SStructNestedChooserTree::TreeViewGenerateRow(TSharedPtr<FNestedChooserTreeEntry> InItem, const TSharedRef<STableViewBase>& OwnerTable)
	{
	...
	void SStructNestedChooserTree::TreeViewGetChildren(TSharedPtr<FNestedChooserTreeEntry> InItem, TArray<TSharedPtr<FNestedChooserTreeEntry>>& OutChildren)
    {
	...
	void SStructNestedChooserTree::TreeViewDoubleClicked(TSharedPtr<FNestedChooserTreeEntry> SelectedObject)
	{
	...
	void SStructNestedChooserTree::DeleteNestedObject()
	{
		...
		if (SelectedItems.Num() == 1)
		{
			...
			else if (UObject* ObjectToDelete = SelectedItems[0]->Object)
			{
				...
				RootChooser->RemoveNestedObject(ObjectToDelete);
			}
		}
	}

	void SStructNestedChooserTree::RenameNestedObject()
	{
		TArray<TSharedPtr<FNestedChooserTreeEntry>>SelectedItems = TreeView->GetSelectedItems();
	...
	void SStructNestedChooserTree::Construct(const FArguments& InArgs)
	{
		ChooserEditor = InArgs._ChooserEditor;
		RootChooser = ChooserEditor->GetRootChooser();
		RootChooser->NestedObjectsChanged.AddRaw(this, &SStructNestedChooserTree::RefreshAll);
		...
		TreeView = SNew(STreeView<TSharedPtr<FNestedChooserTreeEntry>>)
							...
							.OnContextMenuOpening(this, &SStructNestedChooserTree::TreeViewContextMenuOpening)
							.TreeItemsSource(&TreeEntries)
							.SelectionMode(ESelectionMode::Single)
							.OnGenerateRow(this, &SStructNestedChooserTree::TreeViewGenerateRow)
							.OnGetChildren(this, &SStructNestedChooserTree::TreeViewGetChildren)
							.OnMouseButtonDoubleClick(this, &SStructNestedChooserTree::TreeViewDoubleClicked)
							;
	...
	SStructNestedChooserTree::~SStructNestedChooserTree()
	{
		if (RootChooser)
		{
	...
	void SStructNestedChooserTree::RefreshAll()
	{
