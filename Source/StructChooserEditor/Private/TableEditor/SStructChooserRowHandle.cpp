...
namespace UE::StructChooserEditor
{
	...
	TSharedRef<FChooserRowDragDropOp> FChooserRowDragDropOp::New(TSharedPtr<FStructChooserTableViewModel> InViewModel, uint32 InRowIndex)
	{
	...
	void SStructChooserRowHandle::Construct(const FArguments& InArgs, bool bShowImage)
	{
			...
			[
				...
				[
					...
					[
						...
						{
							return ChooserViewModel->GetChooser()->GetDebugTestValuesValid() && RowIndex == ChooserViewModel->GetChooser()->GetDebugSelectedRow() ? EVisibility::HitTestInvisible : EVisibility::Hidden;
						})
	...
	FReply SStructChooserRowHandle::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
	{
	...
	FReply SStructChooserRowHandle::OnDragDetected(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
	{
