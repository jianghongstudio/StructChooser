...
FStructChooserTableViewModel::~FStructChooserTableViewModel()
{
	FTSTicker::RemoveTicker(TickDelegateHandle);
	FCoreUObjectDelegates::OnObjectTransacted.RemoveAll(this);
}

void FStructChooserTableViewModel::RegisterMenus(TSharedPtr<FUICommandList> CommandList)
{
	...
	// Table Context Menu
	UToolMenu* ToolMenu;
	if (ToolMenus->IsMenuRegistered(FStructChooserTableEditor::ContextMenuName))
	{
		ToolMenu = ToolMenus->ExtendMenu(FStructChooserTableEditor::ContextMenuName);
	}
	else
	{
		ToolMenu = UToolMenus::Get()->RegisterMenu(FStructChooserTableEditor::ContextMenuName, NAME_None, EMultiBoxType::Menu);
	}
...
void FStructChooserTableViewModel::OnObjectsTransacted(UObject* Object, const FTransactionObjectEvent& Event)
{
	if (UChooserTable* ChooserTable = Cast<UChooserTable>(Object))
	{
		...
		{
			if (GetCurrentSelectionType() == FStructChooserTableViewModel::ESelectionType::Rows)
			{
	...
	if (UStructChooserRowDetails* RowDetails = Cast<UStructChooserRowDetails>(Object))
	{
...
bool FStructChooserTableViewModel::HandleTicker(float DeltaTime)
{
	if (!QueuedRowTransactions.IsEmpty())
	{
		for (TWeakObjectPtr<UStructChooserRowDetails> RowDetails : QueuedRowTransactions)
		{
...
FStructChooserTableViewModel::FStructChooserTableViewModel(UChooserTable* Chooser)
{
	check(Chooser);

	TickDelegateHandle = FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateRaw(this, &FStructChooserTableViewModel::HandleTicker), 1.f);
	
	RootChooser = Chooser->GetRootChooser();
	CurrentChooser = Chooser;

	FCoreUObjectDelegates::OnObjectTransacted.AddRaw(this, &FStructChooserTableViewModel::OnObjectsTransacted);
}

void FStructChooserTableViewModel::RefreshAll()
{
	...
	else if (CachedSelectionType == ESelectionType::Rows)
	{
		...
		for(const TObjectPtr<UStructChooserRowDetails>& SelectedRow : SelectedRows)
		{
...
bool FStructChooserTableViewModel::MatchesContext(const FTransactionContext& InContext, const TArray<TPair<UObject*, FTransactionObjectEvent>>& TransactionObjectContexts) const
{
	TArray<UObject*> ContainedObjects;
	GetObjectsWithOuter(GetRootChooser()->GetPackage(), ContainedObjects, true);
...
void FStructChooserTableViewModel::PostUndo(bool bSuccess)
{
	SelectRootProperties();
	RefreshAll();
...
void FStructChooserTableViewModel::PostRedo(bool bSuccess)
{
	SelectRootProperties();
	RefreshAll();
	RootChooser->NestedObjectsChanged.Broadcast();
...
void FStructChooserTableViewModel::NotifyPreChange(FProperty* PropertyAboutToChange)
{
}

void FStructChooserTableViewModel::NotifyPostChange(const FPropertyChangedEvent& PropertyChangedEvent, FProperty* PropertyThatChanged)
{
...
void FStructChooserTableViewModel::RemoveDisabledData()
{
...
int32 FStructChooserTableViewModel::MoveColumn(int32 SourceIndex, int32 TargetIndex)
{
	UChooserTable* Chooser = GetChooser();
...
void FStructChooserTableViewModel::UpdateTableColumns()
{
	RefreshTableColumnsDelegate.ExecuteIfBound();
}

void FStructChooserTableViewModel::QueueRowTransaction(UStructChooserRowDetails* RowDetails)
{
	QueuedRowTransactions.Add(RowDetails);
}

void FStructChooserTableViewModel::AddColumn(const UScriptStruct* ColumnType)
{
...
void FStructChooserTableViewModel::RefreshRowSelectionDetails()
{
	...
	ResultPropertyDesc.MetaData.Add({"BaseStruct", FStructChooserBase::StaticStruct()->GetPathName()});
	// Hide bag type picker (FStructFilter rejects Hidden); FStructChooserRowDetails provides Struct-only combo.
	ResultPropertyDesc.MetaData.Add({"StructTypeConst", ""});
	...
	for(int32 SelectedRowIndex : SelectedRowIndices)
	{
		if (Chooser->ResultsStructs.IsValidIndex(SelectedRowIndex) || SelectedRowIndex == ColumnWidget_SpecialIndex_Fallback)
		{
			...
			{
				SelectedRowsCache[SelectedRowsCacheIndex] = TStrongObjectPtr(NewObject<UStructChooserRowDetails>(RootChooser, NAME_None, RF_Transactional | RF_Transient));
			}
			TObjectPtr<UStructChooserRowDetails> Selection = SelectedRowsCache[SelectedRowsCacheIndex].Get();
...
void FStructChooserTableViewModel::UpdateTableRows()
{
	RefreshTableRowsDelegate.ExecuteIfBound();
}

void FStructChooserTableViewModel::DeleteColumn(int32 Index)
{
...
void FStructChooserTableViewModel::DeleteRows(TConstArrayView<int32> RowsToDelete)
{
	const FScopedTransaction Transaction(LOCTEXT("Delete Row Transaction", "Delete Row"));
	DeleteRowsInternal(RowsToDelete, 0);
}

int32 FStructChooserTableViewModel::DeleteRowsInternal(TConstArrayView<int32> InRowsToDelete, int32 RowIndexToRemember)
{
	if (UChooserTable* Chooser = GetChooser())
	{
		...
		TArray<uint32> RowsToDeleteUInt;
		RowsToDeleteUInt.Reserve(RowsToDelete.Num());
		for (int32 RowIndex : RowsToDelete)
		{
			RowsToDeleteUInt.Add(static_cast<uint32>(RowIndex));
		}
		...
		{
			FChooserColumnBase& Column = ColumnData.GetMutable<FChooserColumnBase>();
			Column.DeleteRows(RowsToDeleteUInt);
		}
...
void FStructChooserTableViewModel::MoveRows(TConstArrayView<int32> RowIndices, int32 TargetIndex)
{
...
void FStructChooserTableViewModel::AutoPopulateColumn(int32 ColumnIndex)
{
	if (UChooserTable* Chooser = GetChooser())
	{
...
void FStructChooserTableViewModel::AutoPopulateColumn(FChooserColumnBase& Column)
{
	if (UChooserTable* Chooser = GetChooser())
	{
...
void FStructChooserTableViewModel::AutoPopulateRows(TConstArrayView<int32> RowIndices)
{
	if (UChooserTable* Chooser = GetChooser())
	{
...
void FStructChooserTableViewModel::AutoPopulateRow(int32 Index)
{
	if (UChooserTable* Chooser = GetChooser())
	{
...
bool FStructChooserTableViewModel::CanAutoPopulateColumn(int32 ColumnIndex) const
{
...
bool FStructChooserTableViewModel::CanAutoPopulateRows() const
{
	if (const UChooserTable* Chooser = GetChooser())
	{
...
void FStructChooserTableViewModel::AutoPopulateAll()
{
	if (UChooserTable* Chooser = GetChooser())
	{
...
bool FStructChooserTableViewModel::HasSelection() const
{
	return HasRowsSelected() || HasColumnSelected();
}

bool FStructChooserTableViewModel::HasRowsSelected() const
{
	return CurrentSelectionType == ESelectionType::Rows && !SelectedRows.IsEmpty();
}
	
bool FStructChooserTableViewModel::HasColumnSelected() const
{
...
bool FStructChooserTableViewModel::IsColumnDisabled(int32 ColumnIndex) const
{
	if (const UChooserTable* Chooser = GetChooser())
	{
		if ( Chooser->ColumnsStructs.IsValidIndex(ColumnIndex))
...
bool FStructChooserTableViewModel::AreRowsDisabled(TConstArrayView<int32> RowIndices) const
{
	if (const UChooserTable* Chooser = GetChooser())
	{
...
void FStructChooserTableViewModel::ToggleDisableColumn(int32 ColumnIndex)
{
	if (UChooserTable* Chooser = GetChooser())
	{
...
void FStructChooserTableViewModel::ToggleDisableRows(TConstArrayView<int32> RowIndices)
{
	if (UChooserTable* Chooser = GetChooser())
	{
...
void FStructChooserTableViewModel::DuplicateColumn(int32 ColumnIndex)
{
...
void FStructChooserTableViewModel::DuplicateRows(TConstArrayView<int32> RowIndices)
{
...
bool FStructChooserTableViewModel::HasFallbackSelected()
{
	for(UStructChooserRowDetails* SelectedRow : SelectedRows)
	{
...
bool FStructChooserTableViewModel::CanMoveRowsUp(TConstArrayView<int32> RowIndices)
{
	UChooserTable* Chooser = GetChooser();
...
int32 FStructChooserTableViewModel::GetNumRows() const
{
	if (const UChooserTable* Chooser = GetChooser())
	{
...
bool FStructChooserTableViewModel::CanMoveRowsDown(TConstArrayView<int32> RowIndices)
{
	UChooserTable* Chooser = GetChooser();
...
bool FStructChooserTableViewModel::IsRandomizeColumn(int32 ColumnIndex)
{
	UChooserTable* Chooser = GetChooser();
...
bool FStructChooserTableViewModel::CanMoveColumnLeft(int32 ColumnIndex)
{
	UChooserTable* Chooser = GetChooser();
...
void FStructChooserTableViewModel::MoveColumnLeft(int32 ColumnIndex)
{
...
bool FStructChooserTableViewModel::CanMoveColumnRight(int32 ColumnIndex)
{
	UChooserTable* Chooser = GetChooser();
...
void FStructChooserTableViewModel::MoveColumnRight(int32 ColumnIndex)
{
...
UChooserTable* FStructChooserTableViewModel::CopyRowsInternal(TConstArrayView<int32> RowIndices)
{
...
void FStructChooserTableViewModel::SetChooser(UChooserTable* Chooser)
{
	RootChooser = Chooser->GetRootChooser();
	CurrentChooser = Chooser;
	RefreshAll();
...
UChooserTable* FStructChooserTableViewModel::CopyColumnInternal(int32 ColumnIndex)
{
	UChooserTable* CopyData = NewObject<UChooserTable>(GetTransientPackage());
...
void FStructChooserTableViewModel::CopyColumn(int32 ColumnIndex)
{
...
void FStructChooserTableViewModel::CopyRows(TConstArrayView<int32> RowsToCopy)
{
...
void FStructChooserTableViewModel::PasteInternal(UChooserTable* PastedContent, int32 PasteRowIndex)
{
	UChooserTable* Chooser = GetChooser();
	Chooser->Modify();
...
void FStructChooserTableViewModel::Paste()
{
...
bool FStructChooserTableViewModel::CanPaste() const
{
	// Get the text from the clipboard.
	FString ClipboardText;
	FPlatformApplicationMisc::ClipboardPaste(ClipboardText);
...
void FStructChooserTableViewModel::SelectColumn(UChooserTable* ChooserEditor, int32 Index)
{
	ClearSelectedRows();
	...
	UChooserTable* Chooser = GetChooser();
   	if (Chooser->ColumnsStructs.IsValidIndex(Index))
   	{
   		...
   		{
   			SelectedColumn = TStrongObjectPtr(NewObject<UStructChooserColumnDetails>(RootChooser, NAME_None, RF_Transactional | RF_Transient));
   		}
...
void FStructChooserTableViewModel::ClearSelectedColumn()
{
	UChooserTable* Chooser = GetChooser();
	if (CurrentSelectionType == ESelectionType::Column && SelectedColumn)
	{
...
void FStructChooserTableViewModel::SelectRootProperties()
{
	ClearSelectedColumn();
	ClearSelectedRows();
...
void FStructChooserTableViewModel::SetSelectedRows(const TConstArrayView<int32>& Rows)
{
	ClearSelectedColumn();
	SetCurrentSelectionType(FStructChooserTableViewModel::ESelectionType::Rows);
...
void FStructChooserTableViewModel::SelectRows(const TConstArrayView<int32>& Rows)
{
	SelectRowsDelegate.ExecuteIfBound(Rows);
}

void FStructChooserTableViewModel::RefreshAssetRegistry()
{
...
void FStructChooserTableViewModel::ClearSelectedRows()
{
	SelectRows({});
}

bool FStructChooserTableViewModel::IsRowSelected(int32 RowIndex)
{
...
bool FStructChooserTableViewModel::IsColumnSelected(int32 ColumnIndex)
{
	return (CurrentSelectionType == ESelectionType::Column && SelectedColumn && SelectedColumn->Column == ColumnIndex);
}

void FStructChooserTableViewModel::MakeDebugTargetMenu(UToolMenu* InToolMenu)
{
